...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='vrc.conf'
CONFIGFOLDER='/root/.vrc'
COIN_DAEMON='vrcd'
COIN_CLI='vrc-cli'
...
function get_ip() {
  declare -a NODE_vrc
  for vrc in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_vrc+=($(curl --interface $vrc --connect-timeout 2 -s4 icanhazip.com))
  done
