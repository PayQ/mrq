...
*Setup instructions for a gitian build of vrc using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the vrc
...
Clone the git repositories for vrc and gitian and then checkout the vrc version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/vrc-crypto/vrc.git
cd vrc
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vrc repository
...
To build vrc (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vrc repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vrc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/vrc-crypto/vrc
...
./bin/gbuild --commit vrc=${COMMIT} --url vrc=${URL} ../vrc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vrc=${COMMIT} --url vrc=${URL} ../vrc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vrc=${COMMIT} --url vrc=${URL} ../vrc/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "vrccoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vrc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vrc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vrc-build.assert
```
...
[vrc/gitian.sigs](https://github.com/vrc-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
