...
*Setup instructions for a gitian build of vrq using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the vrq
...
Clone the git repositories for vrq and gitian and then checkout the vrq version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/vrq-crypto/vrq.git
cd vrq
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vrq repository
...
To build vrq (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vrq repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vrq/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/vrq-crypto/vrq
...
./bin/gbuild --commit vrq=${COMMIT} --url vrq=${URL} ../vrq/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vrq=${COMMIT} --url vrq=${URL} ../vrq/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vrq=${COMMIT} --url vrq=${URL} ../vrq/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "vrqcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vrq-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vrq-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vrq-build.assert
```
...
[vrq/gitian.sigs](https://github.com/vrq-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
